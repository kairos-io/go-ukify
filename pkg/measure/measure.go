...
func GenerateSignedPCR(sectionsData SectionsData, rsaKey RSAKey, PCR int, logger *slog.Logger, privKey string) (*types.PCRData, error) {
	data := &types.PCRData{}
	...
	slog.Info("measure calculate output", "out", outputCalculate.Sha256[0].Hash)
	cmd = exec.Command("/usr/lib/systemd/systemd-measure", append([]string{"sign"}, args...)...)
	out, _ = cmd.CombinedOutput()
	outPut := AutoGenerated{}
	json.Unmarshal(out, &outPut)
	slog.Info("measure sign output", "PKFP", outPut.Sha256[0].Pkfp)
	slog.Info("measure sign output", "pol", outPut.Sha256[0].Pol)
	slog.Info("measure sign output", "Sig", outPut.Sha256[0].Sig)
